...
export function ProjectCard({ project }: ProjectCardProps) {
  return (
    <div className="project-card group relative bg-card/80 backdrop-blur-sm rounded-3xl p-8 shadow-lg hover:shadow-2xl transition-all duration-500 border border-border/50 hover:border-blue-500/30 hover:scale-[1.02] hover:-translate-y-2">
      ...
      <div className="relative z-10">
        ...
        <div className="space-y-4">
          <div>
            <h4 className="text-2xl font-bold text-foreground mb-2 group-hover:text-blue-600 transition-colors duration-300">
              {project.title}
            </h4>
            {project.tagline && (
            ...
            <div className="mb-3">
              ...
              <div className="flex flex-wrap gap-2">
                {project.languages.map((lang) => (
                  <span
                    key={lang}
                    className={`px-2 py-1 rounded-md text-xs font-medium border ${getLanguageColor(lang)}`}
                  >
                    {lang}
                  </span>
                ))}
          ...
          <div className="flex gap-3">
            {project.githubUrl && (
              <a
                ...
                className="flex-1 bg-muted hover:bg-muted/80 text-foreground font-medium py-2.5 px-4 rounded-lg transition-all duration-200 text-center text-sm flex items-center justify-center gap-2"
              >
