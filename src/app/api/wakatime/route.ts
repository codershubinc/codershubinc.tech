...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      projects: todayData.projects
        .slice(0, 3)
        .map((p: { name: string; text: string; percent: number }) => ({
          name: p.name,
          text: p.text,
          percent: p.percent,
        })),
      languages: todayData.languages
        .slice(0, 5)
        .map((l: { name: string; text: string; percent: number }) => ({
          name: l.name,
          text: l.text,
          percent: l.percent,
        })),
      editors: todayData.editors
        .slice(0, 3)
        .map((e: { name: string; text: string; percent: number }) => ({
          name: e.name,
          text: e.text,
          percent: e.percent,
        })),
      operating_systems: todayData.operating_systems
        .slice(0, 3)
        .map((os: { name: string; text: string; percent: number }) => ({
          name: os.name,
          text: os.text,
          percent: os.percent,
        })),
